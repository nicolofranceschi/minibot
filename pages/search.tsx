...
import { isPdf,linkToBlob } from 'utils/functions';
...
export default function Search() {
  ...
  const scarica = async (file:any) => {

    const blob = await linkToBlob(file)

    console.log(blob)
    
    download.mutate(blob,{ 
          ...
          <Box sx={{ position: "fixed", width: "100%", top: 0, left: 0, padding: '1rem', backdropFilter: "blur(10px)" }}>
            <Paper sx={{ padding: '1rem', borderRadius: "1rem", display: "flex", flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 2, }}>
              <TextField
                ...
                placeholder="Cerca" value={input} onChange={e => setInput(e.target.value)} />
              <Button sx={{ padding: '1rem' }} variant="contained">
                <FilterListIcon />
              </Button>
            </Paper>
          </Box>
          <Paper sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2, mt: "120px", width: "100%" }}>

          </Paper>
          <Box sx={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 2, mt: "1rem" }}>
            {data && data.map((item: ItemType, i: number) => ( // mostra meno se ci sono piu 5 elementi a schermo
      ...
      {details && (
        <Box>
          ...
          <Stack direction="row" sx={{ gap: 3, flexWrap: "wrap" }}>
            {details.files.map((file, i) => <File url={file.url} name={file.name} key={i} />)}
          </Stack>
          <Button onClick={() => scarica(details.files)} variant='contained' sx={{mt:1 , padding: '1rem'}}>scarica</Button>
        </Box>
      )}
    </Box>
  );
